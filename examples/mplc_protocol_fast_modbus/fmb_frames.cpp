...
{
    // FD 46 10 [min_slave] [max_data_len] [ack_slave] [ack_flag] CRC_L CRC_H — 9 bytes
    std::vector<uint8_t> f;
    f.push_back(FMB_BROADCAST_ADDR);
    f.push_back(FMB_FUNC);
...
std::vector<uint8_t> build_event_config(uint8_t slave_addr, uint8_t type,
                                         ...
                                         uint8_t prio)
{
    // [addr] 46 18 05 [type] [reg_h] [reg_l] [count] [prio] CRC_L CRC_H — 11 bytes
    ...
    f.push_back(count);         // 1 byte — NOT 2!
    f.push_back(prio);
    FmbTransport::append_crc(f);
    return f;
}
...
{
    // FD 46 03 [serial BE 4B] [mb_addr] CRC_L CRC_H — 10 bytes
                          ...
                          uint8_t& out_ack_flag)
{
    ...
    if (sub == FMB_SUB_EVT_NONE) {
        // [slave_id] 46 12 [FLAG] CRC_L CRC_H — 6 bytes
    ...
    // [slave_id] 46 11 [FLAG] [N] [DATA_LEN] [events × N] CRC_L CRC_H
    if (len < 8) return -1;
...
double fmb_event_to_double(const FmbEvent& ev)
{
    switch (ev.payload_len) {
        ...
        case 2: {
            ...
            // Most MR-02m sensor registers are signed (temperature ×10, etc.).
