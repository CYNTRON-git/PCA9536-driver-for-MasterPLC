...
Дополнительно для доступа к `PCA9536` внесены исправления по надежности:

- убраны прямые вызовы `system("i2cset ...")` и `system("gpioset ...")` из hot path;
- добавлен общий Linux helper для `PCA9536` и `USB_POWER`, который использует `open/ioctl/read/write`;
- добавлен межпроцессный `flock` на `/run/lock/sa02m-pca9536.lock`, чтобы не конфликтовать с другими службами при доступе к одной I2C-шине;
- добавлена проверка конфигурационного регистра `0x03` и повторная инициализация выходов перед записью;
- после записи выполняется чтение регистра `0x01` для верификации результата;
- логика `USB_POWER` отделена от `PCA9536`: она по-прежнему работает через GPIO line `268`, но теперь без shell-команд.
...
- `examples/common/ca02m_hw.h` - общий helper Linux-доступа к `PCA9536` и `USB_POWER`;
...
На реальном СА-02м эта шина может одновременно использоваться runtime-сервисом MasterPLC и вспомогательными Linux-компонентами. Поэтому в текущей реализации драйвера доступ к `PCA9536` сериализуется через lock-файл, а запись выполняется только через единый C++ helper, без shell-обёрток.
...
Если нужно убедиться, что драйвер и внешняя служба не лезут в шину одновременно, проверьте наличие lock-файла и владельцев процесса:

```bash
ls -l /run/lock/sa02m-pca9536.lock
pgrep -a mplc
pgrep -a mplc4
```
