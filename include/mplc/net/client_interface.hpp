...
namespace mplc { namespace net {
    ...
    public:
        ...
        explicit client_interface(lib::io_context& ctx_): ctx(ctx_), resolver(ctx) {}
        ...
        void connect(const std::string& host, uint16_t port) {
            resolver.async_resolve(host,
                                   ...
                                   [this](const lib::error_code& ec,
                                       ...
                                       if (!ec) {
                                           m_connection = lib::make_shared<connection_t>(ctx, lib::tcp::socket(ctx));
                                           m_connection->connect_to_server(endpoints,
                                                                           lib::bind(&client_interface::connect_result,
                                                                                     this,
                                                                                     lib::placeholders::_1));
                                       } else {
                                           connect_result(ec);
