...
namespace mplc { namespace net { namespace ssl {
    ...
    public:
        ...
        server_interface(uint16_t port,
                         const lib::ip::address& addr,
                         lib::io_context& _ctx,
                         ssl_context* ssl_ctx_ = nullptr)
            : ctx(_ctx), ssl_ctx(ssl_ctx_ ? ssl_ctx_ : new ssl_context(ssl_context::tlsv13_server)),
              m_acceptor(ctx, lib::tcp::endpoint(addr, port)) {
            if (ssl_ctx_ == nullptr) {
                init_certificate();
            }
        }
        explicit server_interface(uint16_t port, lib::io_context& _ctx, ssl_context* ssl_ctx_ = nullptr)
            : ctx(_ctx), ssl_ctx(ssl_ctx_ ? ssl_ctx_ : new ssl_context(ssl_context::tlsv13_server)),
              m_acceptor(ctx, lib::tcp::endpoint(lib::tcp::v4(), port)) {
            if (ssl_ctx_ == nullptr) {
                init_certificate();
            }
        }
        ...
        void wait_client_connection() {
            m_acceptor.async_accept([this](lib::error_code ec, lib::tcp::socket socket) {
                ...
                if (ec)
                    return;
                auto conn = lib::make_shared<connection_t>(ctx, std::move(socket), *ssl_ctx);
                if (on_connect(conn)) {
    ...
    protected:
        ...
        void init_certificate() const {
            const std::string cert_path = "ssl_cert1.pem";
            const std::string key_path = "ssl_key1.pem";
            if (!boost::filesystem::exists(cert_path) || !boost::filesystem::exists(key_path)) {
                mplc::ssl::create_certificate_x509_rsa(cert_path, key_path, "");
            }
            ssl_ctx->use_certificate_file(cert_path, ssl_context::pem);
            ssl_ctx->use_private_key_file(key_path, ssl_context::pem);
        }
