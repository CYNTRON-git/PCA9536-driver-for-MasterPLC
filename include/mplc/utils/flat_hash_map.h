...
namespace mplc { namespace utils {
    ...
    public:
        ...
        private:
            ...
            hm_iterator(const hm_iterator<OtherContT, OtherIterVal>& other): hm_(other.hm_), idx_(other.idx_) {}

            void advance_past_empty() {
                while (idx_ < hm_->buckets_.size() && key_equal()(hm_->buckets_[idx_].first, hm_->empty_key_)) {
                    ++idx_;
                }
            }
        ...
        using iterator = hm_iterator<flat_hash_map, value_type>;
        using const_iterator = hm_iterator<const flat_hash_map, const value_type>;

    public:
        flat_hash_map(size_type bucket_count = 0,
                      key_type empty_key = {},
        ...
        flat_hash_map(const flat_hash_map& other, size_type bucket_count)
            : flat_hash_map(bucket_count, other.empty_key_, other.get_allocator()) {
        ...
        void rehash(size_type count) {
            ...
            flat_hash_map other(*this, count);
            swap(other);
        }
    ...
    private:
        ...
        const mapped_type& at_impl(const K& key) const {
            return const_cast<flat_hash_map*>(this)->at_impl(key);
        }
        ...
        const_iterator find_impl(const K& key) const {
            return const_cast<flat_hash_map*>(this)->find_impl(key);
        }
