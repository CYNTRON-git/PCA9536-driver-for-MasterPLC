...
#include <mplc/vm/vm_type.h>

namespace mplc { namespace json {
    struct create_msgpack_visitor {
        create_msgpack_visitor(msgpack::object::with_zone& oz,
                               const vm::VmType* type = nullptr,
                               bool use_ref = false,
                               size_t zone_chank = 1024)
            : z(oz.zone), m_zone_chank(zone_chank), m_ref(use_ref) {
        ...
        bool visit_str(const char* str, size_t size) {
            /*if (size > m_limit.str())
