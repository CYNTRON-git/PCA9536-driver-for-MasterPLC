 ...
 *===========================================================================*/
    ...
    if ((xArg) == OpcUa_Null) \
    { \
        OpcUa_Trace(OPCUA_TRACE_LEVEL_WARNING, "<-- ReturnError: Value " #xArg " is OpcUa_Null!\n"); \
        return xStatus; \
    }
#else
    #define OpcUa_ReturnErrorIfNull(xArg, xStatus) \
 ...
 *===========================================================================*/
#if OPCUA_TRACE_ERROR_MACROS
#    define OpcUa_ReturnErrorIfBad(xExpr) \
        {                                                                                                              \
            OpcUa_StatusCode xStatus = xExpr;                                                                          \
            if (OpcUa_IsBad(xStatus)) {                                                                                \
                OpcUa_Trace(OPCUA_TRACE_LEVEL_WARNING, "<--CheckError: " #xExpr " returns 0x%08X\n", xStatus);         \
                return xStatus;                                                                                        \
            }                                                                                                          \
        }
...
// InSAT Взято из пред версии. D 1.04 нет.
#if OPCUA_TRACE_ERROR_MACROS
//InSAT
    #define OpcUa_CheckError(xExpr) \
	do { \
		OpcUa_StatusCode xStatus = xExpr; \
		if (OpcUa_IsBad(xStatus)) \
		{ \
...
#if OPCUA_TRACE_ERROR_MACROS
// InSAT
#    define OpcUa_CheckErrorText(xExpr, text, ...)                                                                     \
        {                                                                                                              \
            OpcUa_StatusCode xStatus = xExpr;                                                                          \
            if(OpcUa_IsBad(xStatus)) {                                                                                 \
                OpcUa_Trace(OPCUA_TRACE_LEVEL_WARNING, #text "<-- " #xExpr " returns 0x%08X\n", __VA_ARGS__, xStatus); \
            }                                                                                                          \
        }
...
#    define OpcUa_CheckErrorText(xExpr, text, ...)                                                                     \
        { OpcUa_StatusCode xStatus = xExpr; }
#endif
// End InSAT Взято из пред версии. D 1.04 нет.
...
// InSAT Взято из пред версии. В 1.04 нет
...
#if OPCUA_TRACE_ERROR_MACROS
//InSAT
    #define OpcUa_ReturnError(xStatus, xText) \
	{ \
		OpcUa_Trace(OPCUA_TRACE_LEVEL_WARNING, "<--ReturnError: %s (0x%08X)\n", xText, xStatus); \
		return xStatus; \
	}
...
// End InSAT Взято из пред версии. В 1.04 нет
 ...
 */
        ...
        OpcUa_Trace(OPCUA_TRACE_LEVEL_CONTENT, "--> " #xModule "::" #xMethod " (0x%08X)\n", (xModule&0x0000FFFF));\
 ...
 */
    ...
    OpcUa_Trace(OpcUa_IsGood(uStatus) ? OPCUA_TRACE_LEVEL_CONTENT : OPCUA_TRACE_LEVEL_WARNING, "<-- \"%s\" = 0x%08X (%s).\n", uStatusMethod, uStatus, OpcUa_IsGood(uStatus)?"GOOD":"BAD");\
    return uStatus;
#else /* OPCUA_TRACE_ERROR_MACROS */
