...
#endif
    /*! @brief Признак работы в режиме Reverse Hello. 
        В режиме Reverse Hello клиент открывает серверный сокет на порту, определенному в PortReverseHello 
        и ожидает подключения от сервера.
    */
    OpcUa_Boolean UseReverseHello;
    /*! @brief Порт для работы в режиме Reverse Hello. 
    При усановленном в  UseReverseHello признаке режима Reverse Hello, клиент открывает серверный сокет на порту, определенному в PortReverseHello 
        и ожидает подключения от сервера.*/
    OpcUa_UInt16 PortReverseHello;
