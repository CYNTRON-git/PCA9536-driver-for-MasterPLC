 ...
 */
/// InSAT
/// @todo посмотреть в спецификации почему отличается from enumeration OpcUa_MessageSecurityMode!
/// возможно должен быть один endpoint и в нем битовой маской определятся доступные messagesecuritymodes.
/// сейчас в сервере для каждой security policy заводится отдельный endpoint для каждого messagesecuritymode.
 ...
 */
OPCUA_EXPORT OpcUa_StatusCode OpcUa_Endpoint_Open(
    OpcUa_Endpoint                              hEndpoint,
    OpcUa_StringA                               sUrl,
    ...
    // InSAT 
    OpcUa_Boolean                               useReverseHello,
    OpcUa_StringA                               reverseHelloCltAddress);
 ...
 */
OPCUA_EXPORT OpcUa_StatusCode OpcUa_Endpoint_GetPeerInfo( OpcUa_Endpoint           a_hEndpoint,    OpcUa_Handle             a_hContext,    OpcUa_String*            a_pPeerInfo);
