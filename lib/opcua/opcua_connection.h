...
struct _OpcUa_Connection
{
    ...
    /// <summary>
    /// InSAT 
    /// Используем ReverseHello для сединения как определно в спецификации OPC UA 1.04 part 6
    /// </summary>
    OpcUa_Boolean UseReverseHello;

    OpcUa_UInt16 PortReverseHello;

    OpcUa_UInt32 TransportTimeout;

    /// <summary>
    /// Используется при ReverseHello. ServerUri URI сервера, пришедшее при ReverseHello от сервера. 
    /// </summary>
    OpcUa_String ServerURI;

    /// <summary>
    /// Используется при ReverseHello. URI сервера, задаваемое при настройке клиента. Используется при разборе пакета ReverseHello
    /// </summary>
    OpcUa_String CltServerURI;
